...
FLAGS = {
    "esrd_dialysis": r"DIAGNOSIS//ICD//(10//(N186|Z992|Z4931|Z4932)|9//(5856|V4511|V560|V568))$",
    "any_dialysis_incl_acute": r"DIAGNOSIS//ICD//(10//(N186|Z992|Z4931|Z4932)|9//(5856|V4511|V560|V568))$|PROCEDURE//ICD//(10//5A1D|9//3995)",
    "palliative_hospice": r"DIAGNOSIS//ICD//(10//Z515|9//V667)$|HOSPITAL_DISCHARGE//HOSPICE",
}
...
out["age_years_quantiles_rows"] = {q: float(d["age"].quantile(q)) for q in (0.0, 0.01, 0.5, 0.99, 1.0)}  # type: ignore[arg-type]
out["rows_age_lt_18"] = int((d["age"] < 18).sum())
out["rows_by_event"] = {}
for evt in d["event"].unique().to_list():
    de = d.filter(pl.col("event") == evt)
    ...
    rec["auroc_24h_all"] = float(roc_auc_score(y[ok], p[ok]))  # type: ignore[assignment]
    if evt == "icu_admission":
        rec["at_risk_rows_scored_while_in_icu"] = int(de.filter(pl.col("in_icu") == 1)["y@24h"].is_not_null().sum())
    for k in FLAGS:
        f = de[k].to_numpy()
        m = ok & ~f
        rec[k] = {  # type: ignore[assignment]
    ...
    out["rows_by_event"][evt] = rec  # type: ignore[index]
