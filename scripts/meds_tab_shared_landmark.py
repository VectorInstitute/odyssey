...
def assert_no_incomplete_tabularize_outputs(tab_out: Path) -> None:
    """Fail loud on a zero-byte ``.npz`` under ``tab_out``, or a stale ``.lock``.
    ...
    ``fp.is_file()``. A ``.npz`` truncated by a killed/paused worker (a
    real incident: SIGSTOP mid-write during a 2026-08-23 disk-pressure
    pause) reads as "already done" under ``do_overwrite=False`` and is
    silently skipped forever, not regenerated on resume. This function is
    the only thing standing between that silent gap and a 3-gate trust
    check that assumes every file is real.
    ...
    """
    zero_byte = sorted(p for p in tab_out.rglob("*.npz") if p.stat().st_size == 0)
    ...
    if zero_byte:
        raise RuntimeError(
            f"{len(zero_byte)} zero-byte .npz file(s) under {tab_out} -- these "
            "read as complete to MEDS-Tab's own rwlock (default_file_checker "
            "only validates .parquet content) and will NOT be regenerated by "
            "a do_overwrite=False resume. Delete them and re-run the "
            "tabularize-time-series (or -static) stage once more before "
            f"trusting this output: {[str(p) for p in zero_byte]}"
        )
