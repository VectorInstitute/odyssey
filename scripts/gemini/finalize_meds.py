...
#: Overridable with ``GEMINI_OUT_DIR`` so a test never writes here. The
#: banked summaries are tracked in git, hold the real full-scale counts,
#: and can only be regenerated by a multi-hour run on the GEMINI node --
#: and a smoke-scale run's suppressed counts ("<6", "0") overwrite them
#: with a file that reads as a valid summary saying GEMINI has no data.
#: That happened on 2026-09-10, caught by an mtime rather than a failure.
OUT_DIR = Path(
    os.environ.get("GEMINI_OUT_DIR") or Path(__file__).resolve().parent / "out"
)
SUMMARY_PATH = OUT_DIR / "finalize_summary.json"
