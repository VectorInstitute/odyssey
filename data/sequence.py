...
class SequenceGenerator:
    ...
    def time_delta_tokens(self) -> list:
        ...
        return  [f'W_{i}' for i in range(0, 4)] + \
            [f'M_{i}' for i in range(0, 13)] + ['LT']
    
    @property
    def special_tokens(self) -> list:
        """Gets the special tokens."""
        return [self.pad_token, self.mask_token, self.start_token, self.end_token] + \
    ...
    def _get_encounters_age(
        ...
        ages = [
            (parser.parse(e_date).replace(tzinfo=None) - birth_date).days // \
                365 for e_date in encounter_dates
        ]
    ...
    def _calculate_intervals(self, encounter_row: pd.Series) -> pd.Series:
        ...
        for i in range(len(start_times) - 1):
            ...
            if days < 0:
                if not start_id in eq_encounters.keys():
                    eq_encounters[start_id] = []
                if not end_id in eq_encounters.keys():
    ...
    def _edit_datetimes(
        ...
        if concept_name == 'lab':
            for name in [
                'encounter_ids', 'lab_dates', 'lab_codes', 'binned_values', 
                'lab_values', 'lab_units'
            ]:
        ...
        for i, date in enumerate(row[date_column]):
            encounter_index = encounter_row['encounter_ids'].index(row['encounter_ids'][i])
    ...
    def _concat_concepts(
        ...
        for i in range(len(procedures)):
            ...
            encounter_order = {
                encounter: i for i, encounter in enumerate(encounters.iloc[i]['encounter_ids'])
            }
    ...
    def _add_tokens(self, row: pd.Series, encounter_row: pd.Series) -> pd.Series:
        ...
        for event, event_encounter in zip(events, events_encounters):
            if event_encounter != prev_encounter and \
                event_encounter not in eq_encounters.keys():
        ...
        assert len(event_tokens) == len(age_tokens) == len(time_tokens) == \
            len(visit_segments) == len(position_tokens)
    ...
    def _truncate_or_pad(self, row: pd.Series) -> pd.Series:
        ...
        for key in [
            'event_tokens', 'age_tokens', 'time_tokens', 'visit_tokens', 'position_tokens'
        ]:
    ...
    def create_patient_sequence(self) -> None:
        ...
        procedures = procedures.apply(
            lambda row: self._edit_datetimes(row, encounters.iloc[row.name], 'proc'), axis=1
        )
        medications = medications.apply(
            lambda row: self._edit_datetimes(row, encounters.iloc[row.name], 'med'), axis=1
        )
        labs = labs.apply(
            lambda row: self._edit_datetimes(row, encounters.iloc[row.name], 'lab'), axis=1
        )
        # combine events
        combined_events = self._concat_concepts(procedures, medications, labs, encounters)
        ...
        output_columns = [ 'patient_id', 'num_visits', 'deceased', 'death_after_start', 
                           'death_after_end', 'length', 'token_length',
                           'event_tokens', 'age_tokens', 'time_tokens', 
                           'visit_tokens', 'position_tokens'
                        ]
