...
11. Concept-set widening (built as `task_set="v3"`: 11 added LOINC-keyed concepts, electrolytes/glucose/anemia/coagulation/acidosis/sustained MAP hypotension), plus leakage metrics (built: `odyssey/inference/leakage.py`, CTL capacity-controlled + ICL matrix, per-run; not yet run on a checkpoint)
