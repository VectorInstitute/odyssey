...
**What success looks like.** On patients the model has never seen: strong set-based forecasting in every event family, close to an unconstrained model of the same size (the bottleneck costs little); event survival curves whose calibration and discrimination at fixed horizons match or beat bespoke single-task models; concept probabilities that track the patient's real state; concept interventions that move forecasts in the clinically expected direction; and all of it holding across hospitals.
...
12. Bundle-aware forecasting: permutation-invariant loss within same-timestamp bundles and family-balanced loss weighting, so medications, procedures, diagnoses and billing are learned and scored as sets rather than drowned out by labs
13. Time-to-event: a hazard head for time to the next bundle (the model conditions on time but does not yet predict it), and survival curves for key events (vasopressor start, ICU transfer, AKI, death) evaluated with time-dependent AUROC/Brier at fixed horizons against bespoke single-task baselines
14. Bundle-level set prediction head and hierarchical ICD (category, then code) for discharge diagnoses
