...
def test_antibiotic_route_exclude_misses_real_mimic_route_abbreviations() -> None:
    """ANTIBIOTIC_ROUTE_EXCLUDE only matches route *words*, not MIMIC-IV's
    real route *abbreviations* -- confirmed on a real held-out shard: 174
    ...
    # These currently FAIL -- the regex does not yet exclude MIMIC-IV's real
    # route abbreviations, only full words like "nasal"/"rectal". Expresses
    # the desired (post-fix) behavior, not what the code does today.
